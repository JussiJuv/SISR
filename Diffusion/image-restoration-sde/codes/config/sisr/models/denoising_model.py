...
class DenoisingModel(BaseModel):
    def __init__(self, opt):
        ...
        if opt.get('space_mode') == 'latent':
            self.netL = networks.define_L(opt).to(self.device)
            self.netL.eval()

            self.encode = self.netL.encode
            self.decode = self.netL.decode

            stats = torch.load(opt['latent_config']['stats_path'])
            self.latent_mean = stats['mean'].view(1, 4, 1, 1).to(self.device)
            self.latent_std = stats['std'].view(1, 4, 1, 1).to(self.device)

    def feed_data(self, state, LQ, GT=None):
        if self.opt.get('space_mode') == 'latent':
            with torch.no_grad():
                LQ_pix = (LQ.to(self.device) + 1.0) * 0.5 
                z_lq, self.hidden_lq = self.netL.encode(LQ_pix.to(self.device))
                #z_lq, self.hidden_lq = self.netL.encode(LQ.to(self.device))
                self.condition = (z_lq - self.latent_mean) / self.latent_std
                
                self.state = state.to(self.device)
                
                if GT is not None:
                    z_gt, _ = self.netL.encode(GT.to(self.device))
    ...
    def test(self, sde=None, save_states=False):
        ...
        self.model.eval()
        with torch.no_grad():
            # 1. Reverse SDE to get the reconstructed latent
            latent = sde.reverse_sde(self.state, save_states=save_states)
            
            if self.opt.get('space_mode') == 'latent':
                # 2. Denormalize the latent (Inverse of (z - mean) / std)
                # Use the stats you calculated with that script
                latent = latent * self.latent_std + self.latent_mean
                
                # 3. Decode with the hidden skip-connection from the LQ image
                # Flow matching shows: self.decode(latent, h_lr)
                self.output = self.decode(latent, self.hidden_lq)
                
                # 4. Clamp to [0, 1] as seen in Flow Matching eval_loop.py
    ...
    def save(self, iter_label):
        ...
        #self.save_network(self.ema.ema_model, "EMA", 'latest')
        if hasattr(self, 'ema'):
